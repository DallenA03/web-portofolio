      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {filteredProjects.map((project, idx) => (
            <PixelCard key={idx} className="group h-full flex flex-col hover:-translate-y-2 transition-transform">
              <div className={`w-full h-48 border-2 border-black mb-4 ${project.color} flex items-center justify-center p-4 relative overflow-hidden`}>
                <div className="bg-white border-2 border-black p-4 font-black text-xl shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
                  {project.title}
                </div>
              ...
              <h4 className="text-xl font-black mb-2 uppercase">{project.title}</h4>
              <p className="text-sm font-bold text-gray-700 flex-grow mb-4 leading-snug">
                {project.desc}
              </p>
              <div className="flex flex-wrap gap-2">
                {project.tags.map(tag => (
                  <span key={tag} className="text-[10px] uppercase font-black px-2 py-1 bg-gray-100 border border-black">
                    #{tag}
                  </span>
                ))}
              </div>
